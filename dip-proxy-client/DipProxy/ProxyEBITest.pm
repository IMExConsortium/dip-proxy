...
sub proxyEBI{
    ...
    print "proxyEBI: $operation, $ac, $ns, detail is $detail, ncbitaxid is $ncbitaxid\n";

    my $soap_server = SOAP::Lite
            ->uri($proxy_conf{EBI_URI_TEST})
            ->proxy($proxy_conf{EBI_SRV_TEST}, timeout=>300)
    ...
    #print "DipDb/proxyEBI: dataset_out is below: \n$dataset_out\n";
    my ($soap_out, $fault) = checkProxySoapOut($operation, $dataset_out, $ac, $ns, $detail,
                                     ++$testnum, $ncbitaxid);

    #print "DipDb/proxyEBI: after checkProxySoapOut: soap_out is below: \n$soap_out\n";

    if( $soap_out eq "0" ){
        return ($soap_out, $fault);
    }

    #print "DipDb/proxyEBI: soap_out is below: \n$soap_out\n";

    if( $operation eq "getPicrList" && $detail eq "full" && defined($ncbitaxid) && $ncbitaxid ne "0"){
...
sub checkProxySoapOut{
    ...
    if( $retest == 1 ){
        proxyEBI("trash", $operation, $ac, $ns, $detail, $testnum, $ncbitaxid);
    }
    ...
    foreach my $xref(@$node_xrefs_ref){
        ...
        if( $xref->{node}->{xrefList}->{xref}->{ac} eq "-3"){
            ...
            if($xref->{ns} =~ /uniprot/i ){
                ($unknown_dataset, $fault) = proxyEBI("trash", "getUniprot", $xref->{ac}, "uniprot", "stub", 0, "");
            }
            if($xref->{ns} =~ /refseq/i){
                my $proxyNCBI = DipProxy::ProxyNCBI->new();
                
                ($unknown_dataset, $fault) = $proxyNCBI->proxyNCBI("getRefseq", $xref->{ac}, "refseq", "stub", 0, "");
