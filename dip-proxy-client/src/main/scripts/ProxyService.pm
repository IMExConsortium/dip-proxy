...
#
# ProxyEBI.pm: 
#   object method proxyEBI ( $provider, service, $ac, $ns, $detail, $format, 
#                            $testnum, $ncbitaxid )           
#
#   required args: $provider, $service, $ac, $ns, $detail, $format, $testnum
...
my $conf_file = "proxyService.conf";
my %proxy_conf;
...
sub proxyService {
    my $self = shift;
    my ($provider, $service, $ac, $ns, $detail, $format, $testnum, $ncbitaxid) = @_;

    my $soap_server = SOAP::Lite
        ->uri($proxy_conf{Proxy_URI})
        ->proxy($proxy_conf{Proxy_SRV}, timeout=>300)
        ->default_ns($proxy_conf{Proxy_RNS})
    ...
    $dataset_out =$soap_server->getRecord(
        SOAP::Data->name('provider')->type(string=>"$provider"),
        SOAP::Data->name('service')->type(string=>"$service"),
    ...
    #print "ProxyService.pm: dataset_out is \n$dataset_out\n";

    my ($soap_out, $fault) = checkProxySoapOut($provider, $service, 
            $dataset_out, $ac, $ns, $detail, $format, ++$testnum, $ncbitaxid);

    #print "ProxyService.pm: after checkProxySoapOut: soap_out is \n$soap_out\n";
...
sub checkProxySoapOut{
    my ( $provider, $service, $datasetOut, $ac, $ns, $detail, $format, 
         $testnum, $ncbitaxid ) = @_;

    my $retest = 0;

    if( $format eq "native" || $format eq "both" ) {
        $datasetOut =~ s/<S:Envelope.*?><S:Body>//;
    ...
    if( $retest == 1 ) {
        proxyService( "trash", $provider, $service, $ac, $ns, $detail, 
                      $format, $testnum, $ncbitaxid );
    }
...
sub filterPicrDataset {
    ...
    my $data = $simple->XMLin( $dataset_string, KeepRoot=>1, KeyAttr=>[], 
                               ParserOpts=>['XML::LibXML'] );
    ...
    foreach my $xref( @$node_xrefs_ref ) {
        ...
        if( $xref->{node}->{xrefList}->{xref}->{ac} eq "-3" ) {
            ...
            if($xref->{ns} =~ /uniprot/i ) {
                ($unknown_dataset, $fault) = proxyService(
                                    "trash", "EBI", "uniprot", $xref->{ac}, 
                                    "uniprot", "base", "dxf", 0, "");
            }
